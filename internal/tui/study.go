...
// tryLoadCachedQuestions shows the stored questions when AI is disabled, and
// falls back to plain recall when the card has none.
func (m Model) tryLoadCachedQuestions(content string) (tea.Model, tea.Cmd) {
	m, ok := m.loadStoredQuestions(content)
	if !ok {
		m.state = stateRecall
	}
	return m, nil
...
// loadStoredQuestions moves the model into stateAIQuestions using the questions
// stored for the current card, and reports whether any were found. Questions
// written by hand and questions generated by AI are stored the same way, so
// both arrive here; a note edited since they were stored is flagged rather than
// hidden, because without AI there is nothing to regenerate them with.
func (m Model) loadStoredQuestions(content string) (Model, bool) {
	card := m.currentCard()
	if card == nil {
		return m, false
	}
